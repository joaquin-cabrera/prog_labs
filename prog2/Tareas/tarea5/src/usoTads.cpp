...
ArregloDoubles longitudesCaminosMasCortos(nat v, TGrafo g){ 
    ...
    cp = insertarEnCP(v, 0, cp);

    while(!estaVaciaCP(cp)){

        nat vertice = prioritario(cp);
        double edgev = prioridad(vertice, cp);
    
        double aux = edgev;
        s[vertice] = edgev;

        cp = eliminarPrioritario(cp);
        TIterador iter = vecinos(vertice, g);

        while(estaDefinidaActual(iter)){

            if(s[actualEnIterador(iter)] == 0){ 

                double dr = distancia(actualEnIterador(iter), vertice , g)  +  aux;

                if(!estaEnCP(actualEnIterador(iter), cp))
                    cp = insertarEnCP(actualEnIterador(iter), dr , cp );

                else if (dr < prioridad(actualEnIterador(iter), cp))
                    cp = actualizarEnCP(actualEnIterador(iter),  dr , cp );
            }
